...
namespace sme_sha512
{
    ...
    public class CoreOpt : SimpleProcess
    {
        ...
        enum States { idle, init, load0, load1, load2, load3, load4, load5, prep, pre_update0, pre_update1, update, post, flush };
        States state = States.idle;
        ...
        protected override void OnTick()
        {
            switch (state)
            {
                ...
                case States.update:
                    bramwr.Enabled = i < 80;
                    bramwr.Address = i;
                    ulong temp1 = h + S3(e) + F1(e, f, g) + k[i-2] + bramrd.Data;
                    ulong temp2 = S2(a) + F0(a, b, c);
